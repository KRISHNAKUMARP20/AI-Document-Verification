...
3. Provide a JSON response EXACTLY matching this schema (do NOT use other keys in extractedFields):
{
  "extractedFields": {
  ...
  const resultPayload = {
    ...
    activityItem: {
      ...
      ocrExtracted: {
        recipientName: ocrFields.fullName || doc.title,
        idNumber: certNumber,
        institution: ocrFields.issuingAuthority || 'Accredited Authority',
        issueDate: ocrFields.dateOfIssue || new Date().toLocaleDateString(),
      },
      forensicChecks: [],
    ...
    certificate: !isTampered ? {
      ...
      issuedToName: ocrFields.fullName || doc.title,
      issuerAuthority: ocrFields.issuingAuthority || 'Accredited Government Authority',
...
async function startServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`AI Document Verification Server running on port ${PORT}`);
  });
}
